...
void Localiza_Paths(Listas *inicio){
	...
	while(aux3 != NULL){
		...
		while(aux1 != NULL){
			...
			if(aux3->vemc == 'G')
				assert(aux3->vaic == 'A');	//Garante que toda interc. que sai de um gerador chegue em um adaptador.
			if(aux3->vemc == 'A')
				assert(aux3->vaic == 'C');	//Garante que toda interc. que sai de um adaptador chegue em uma cidade.
